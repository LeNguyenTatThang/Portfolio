...
const ItemHovered = ({ data, type }: ItemHoveredProps) => (
    <div className="flex flex-col items-center gap-y-1 text-xs">
        ...
        <span className="text-blue-600">{`${Math.floor(data?.maxWpm.raw)} raw`}</span>
        <span className="text-blue-600">{`${Math.floor(data?.maxWpm.acc)}% acc`}</span>
        <span className="text-blue-600">{`${Math.floor(data?.maxWpm.consistency)}% con`}</span>
